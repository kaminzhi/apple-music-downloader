...
func rip(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	if err := os.MkdirAll(sanAlbumFolder, os.ModePerm); err != nil {
		fmt.Printf("Failed to create album directory %s: %v\n", sanAlbumFolder, err)
	}
	...
	errorsChan := make(chan downloadError, len(selected))
	...
	for trackNum, track := range meta.Data[0].Relationships.Tracks.Data {
		...
		if error := sem.Acquire(ctx, 1); error != nil {
			fmt.Printf("Failed to acquire semaphore: %v\n", error)
			wg.Done()
			continue
		}

		go func(trackNum int, track structs.TrackData) {
			defer wg.Done()
			defer sem.Release(1)
			...
			mu.Lock()
			if counter.Error > 0 {
				errorsChan <- downloadError{trackNum: trackNum, err: fmt.Errorf("download failed")}
			}
			mu.Unlock()
		}(trackNum, track)
	}
	...
	if len(downloadErrors) > 0 {
		for _, de := range downloadErrors {
			fmt.Printf("Track %d download failed: %v\n", de.trackNum, de.err)
		}
		return fmt.Errorf("Encountered %d errors during download", len(downloadErrors))
	}
	return nil
}
